 ...
 * @Last Modified time: 2020-03-24 20:53:01
 */ 
?>
<!-- DataTables -->
...
<script>
  ...
  var table = $('#dt').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url('admin/tracemail/dt_status')?>",
    "order": [[0,'DESC']],
    "columnDefs": [
    { "targets": 0, "name": "id", 'searchable':false, 'orderable':true},
    { "targets": 1, "name": "slug_status", 'searchable':true, 'orderable':true},
    { "targets": 2, "name": "nama_status", 'searchable':true, 'orderable':true}, 
  ...
  function reload_table()
    {
     ...
     $(document).ready(function(){
        $('#add').submit(function(e){
            e.preventDefault(); 
                 $.ajax({
                     url:'<?php echo base_url('admin/tracemail/status_surat_save')?>',
                     type:"post",
    ...
    $(document).ready(function(){
          $(document).on('click', '#show_data', function(e){
           ...
           var id = $(this).data('id'); 
           $.ajax({
                url: '<?php echo base_url('admin/tracemail/data_show/status_surat')?>',
                type: 'POST',
                data: {
                ...
                success: function(data){
                    console.log(data); 
                        $.each(data,function(k,v){ 
                            $('[name="nama_status"]').val(v.nama_status);  
                            $('[name="id"]').val(v.id);
                        });

 ...
 * @Last Modified time: 2020-03-17 16:38:27
 */ 
?>
<!-- DataTables -->
...
<script>
  ...
  var table = $('#dt').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url('admin/tracemail/dt_unit')?>",
    "order": [[0,'DESC']],
    "columnDefs": [
    { "targets": 0, "name": "id", 'searchable':false, 'orderable':true},
    { "targets": 1, "name": "kode_unit", 'searchable':true, 'orderable':true},
    { "targets": 2, "name": "nama_unit", 'searchable':true, 'orderable':true}, 
  ...
  function reload_table()
    {
     ...
     $(document).ready(function(){
        $('#add').submit(function(e){
            e.preventDefault(); 
                 $.ajax({
                     url:'<?php echo base_url('admin/tracemail/unit_save')?>',
                     type:"post",
      ...
      $(document).ready(function(){
          $(document).on('click', '#show_data', function(e){
           ...
           var id = $(this).data('id'); 
           $.ajax({
                url: '<?php echo base_url('admin/tracemail/data_show/unit')?>',
                type: 'POST',
                data: {
                ...
                success: function(data){ 
                        $.each(data,function(k,v){ 
                            $('[name="kode_unit"]').val(v.kode_unit); 
                            $('[name="nama_unit"]').val(v.nama_unit);   
                            $('[name="id"]').val(v.id);
                        });
      ...
      $("body").on("click",".delete",function(){
        Swal.fire({
              ...
              if (result.value) {          
                $.post('<?=base_url("admin/tracemail/delete_auto/unit")?>',
                {
                  '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
                  id : $(this).data('del'), 
                },
